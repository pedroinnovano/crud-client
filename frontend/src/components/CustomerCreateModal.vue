<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-md p-6 rounded-xl shadow-xl relative">
      <h2 class="text-xl font-semibold mb-4">Adicionar Contato</h2>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <p v-if="success" class="text-green-600 text-sm mt-4">Contato criado com sucesso!</p>
      </form>
    </div>
  </div>
</template>
...
async function handleSubmit() {
  ...
  try {
    ...
    apiError.value = 'Erro ao criar contato. Tente novamente.'
  } finally {
    loading.value = false
  }
}
